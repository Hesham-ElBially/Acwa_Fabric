...
lakehouse_path = "Files/Ingestion/khalladi/"
#abfss://6bd9efc5-5c69-4694-a489-f60abf43b99f@onelake.dfs.fabric.microsoft.com/d7eae95f-788c-4a38-8021-4351318c2114/Files/Ingestion/khalladi
#abfss://6bd9efc5-5c69-4694-a489-f60abf43b99f@onelake.dfs.fabric.microsoft.com/0025dad5-4ab8-4f25-828c-1ef9a0600fb9/Files/Ingestion/khalladi
# 1. Load the files using the binaryFile reader
#    (It generates 'path', 'length', 'modificationTime', and 'content')
file_df = spark.read.format("binaryFile") \
    .option("pathGlobFilter", "*") \
    .option("recursiveFileLookup", "true") \
    .load(lakehouse_path)

# 2. **CRITICAL STEP:** Select only the metadata columns (drop the 'content' column)
file_metadata_df = file_df.select("path", "length", "modificationTime")

# 3. Get the file count and total size
file_count = file_metadata_df.count()
total_size_bytes = file_metadata_df.agg({"length": "sum"}).collect()[0][0]
...
lakehouse_path = "Files/khalladi/1-min/2025/11"
...
# khalladi/1-min/           237.30 GB   314,812 file
# khalladi/Data/            
# khalladi/Historical/      333.62 GB         6 file
# khalladi/Signals/           1.26 GB       513 file
# ----------------------------------------
#                           572.18 GB

# 237.3 GB | 314,812 file ===>>> 22.22 GB parquet 9.4% | 536,694,255 record ===>>> 0.82 GB 536,694,253 (0.35%)
#----------------------------------------------------------------------------------------------------------------
# Total number of files in Files/khalladi/1-min/2025/07: 67,971 files, this is the size: (22.48 GB) |  52,207,435 record 0.77 GB
# Total number of files in Files/khalladi/1-min/2025/08: 78,860 files, this is the size: (77.39 GB) | 174,514,104 record 3.03 GB
# Total number of files in Files/khalladi/1-min/2025/09: 87,926 files, this is the size: (72.04 GB) | 162,483,546 record 5.16 GB
# Total number of files in Files/khalladi/1-min/2025/10: 80,055 files, this is the size: (65.39 GB) | 147,489,170 record 7.09 GB
# Total number of files in Files/khalladi/1-min/2025/11: 75,508 files, this is the size: (61.57 GB) | 138,911,832 record 8.92 GB


# 333.62 GB |      6 file ===>>> 15.13 GB parquet 4.5% | 2,328,539,680 record ===>>> 3.37 GB 2,328,539,680 (1.01%)
#------------------------------------------------------------------------------------------------------------------
# Total number of files in Files/khalladi/Historical   :      6 files, this is the size: (333.62 GB)|   2,328,539,680 record 15.13 GB
####################################################################################################################### 
# 570.92 GB | 314,818 file ===>>> 15.13 GB parquet 4.5% | 2,865,233,933 record ===>>> 4.19 GB (0.007%)
#######################################################################################################################
...
table_name = "IIoT_Bronze_lakehouse.khalladi.iot_khalladi"
...
# ------------------------------------------------------------
lakehouse_folder = "Files/khalladi/1-min/2025/11"
...
# ------------------------------------------------------------
target_table = "IIoT_Bronze_lakehouse.khalladi.iot_khalladi"
(
    df.write
        .format("delta")
...
CSV_FOLDER_PATH = "Files/khalladi/Historical"
...
target_table = "IIoT_Bronze_lakehouse.khalladi.iot_khalladi_csv"
...
source_table = "IIoT_Bronze_Lakehouse.khalladi.iot_khalladi"
...
df_transformed = df_transformed.withColumn("signalDate", to_date(col("SignalDTTM"))).filter(col("signalDate") == "2025-07-13")#.limit(10)
...
df_transformed = df_transformed.withColumn("value_DateTime",
    when(
        unix_timestamp(value_as_string, "yyyy-MM-dd'T'HH:mm:ss'Z'").isNotNull(),  #2025-08-05T02:02:21Z
        unix_timestamp(value_as_string, "yyyy-MM-dd'T'HH:mm:ss'Z'").cast("timestamp")
    )
)
...
# 9️⃣ Drop unwanted columns
columns_to_drop = ["Id", "timeField","_id","Quality", "Value", "ValueDateAndTime", "InstrumentID", "Quality_Text", "SignalId", "Timestamp", "source_file_path", "value", "IsAlarm", "quality", "Host", "_corrupt_record"]
df_final = df_transformed.drop(*[c for c in columns_to_drop if c in df_transformed.columns])
...
spark.sql("""
    DELETE FROM IIoT_Silver_lakehouse.khalladi.iot_khalladi
    WHERE signalDate IN ('2025-11-18')
""")
...
spark.sql("""
    INSERT INTO IIoT_Bronze_lakehouse.khalladi.part_001 (instrumentID,
			plant,
			section,
			block,
			asset,
    ...
    SELECT instrumentID,
			...
			timestamp,
			timeField
    FROM IIoT_Bronze_lakehouse.khalladi.part_006
""")
