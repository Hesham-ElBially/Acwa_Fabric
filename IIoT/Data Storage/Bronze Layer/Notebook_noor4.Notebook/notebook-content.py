...
lakehouse_path = "Files/noor4/1-min/2025/11"
...
# 276.37 GB | 274,004 file ===>>> 5.61 GB 637,730,904 parquet (2.03%) | 637,730,904 record ===>>>  GB  (3.12%)
#----------------------------------------------------------------------------------------------------------------
# Total number of files in Files/noor4/1-min/2025/08: 174,107 files, this is the size: (181.56 GB) | 418,918,797 record 3.71 GB
# Total number of files in Files/noor4/1-min/2025/09:  69,198 files, this is the size: ( 64.84 GB) | 149,687,624 record 5.02 GB
# Total number of files in Files/noor4/1-min/2025/10:  30,699 files, this is the size: ( 29.97 GB) |  69,124,483 record 5.61 GB
# Total number of files in Files/noor4/1-min/2025/11: 113,669 files, this is the size: (108.25 GB) | 264,224,911 record 3.28 GB

# Total number of files in Files/noor4/Historical   :       4 files, this is the size: (437.44 GB) | 2,661,675,517 record 24.36 GB

# MongoDB extraction
#----------------------------------------------------------------------------------------------------------------
# Total number of files in Files/MongoDB/noor4/     :       2 files, this is the size: ( 81.15 GB) | 274,893,764 record  GB
...
table_name = "IIoT_Bronze_lakehouse.noor4.iot_noor4_11"
...
# ------------------------------------------------------------
lakehouse_folder = "Files/noor4/1-min/2025/11"
...
df = (
   spark.read.format("json")
        .option("recursiveFileLookup", "true")
        .option("multiLine", "true")  # if your JSONs are single-line
...
# ------------------------------------------------------------
target_table = "IIoT_Bronze_lakehouse.noor4.iot_noor4_11"
df.write \
    .format("delta") \
    .mode("append") \
...
print(f"Ensure that the table '{target_table}' exists and is a Delta Lake table.")
...
CSV_FOLDER_PATH = "Files/noor4/Historical"
...
target_table = "IIoT_Bronze_lakehouse.noor4.iot_noor4_csv"
...
source_table = "IIoT_Bronze_lakehouse.noor4.iot_noor4"
...
df_transformed = df_transformed.withColumn("value_Text", when((col("value_Number").isNull()) & (col("value_DateTime").isNull()) & (col("Value").isNotNull()), col("Value")))
...
target_table = "IIoT_Silver_Lakehouse.noor4.iot_noor4"
#overwrite   append
df_final.write \
     .format("delta") \
     .mode("append") \
...
# 3️⃣ Read the source table
source_table = "IIoT_Bronze_lakehouse.noor4.iot_noor4_11"
df = spark.read.table(source_table)
...
when((col("value_Number").isNull()) & (col("value_DateTime").isNull()) & (col("Value").isNotNull()), 
...
# 9️⃣ Drop unwanted columns
columns_to_drop = ["_id","Id", "InstrumentID", "Quality", "Quality_Text", "SignalId", "Timestamp", "source_file_path", "Value", "IsAlarm", "quality", "Host", "_corrupt_record","timeField","ValueDateAndTime"]
df_final = df_transformed.drop(*[c for c in columns_to_drop if c in df_transformed.columns])
...
spark.sql("""
    DELETE FROM  IIoT_Silver_lakehouse.noor4.iot_noor4
    WHERE to_date(timeField) IN ('2024-11-20')
""")
