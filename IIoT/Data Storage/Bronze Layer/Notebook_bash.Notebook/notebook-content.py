...
lakehouse_path = "Files/iot-raw-data-json/bash/1-min/2026/01"
...
#===========================================================
#===========================================================
# 867.77 GB | 1,015,158 file ===>>> 29.80 GB parquet 3.43% | 2,074,181,993 record ===>>> 8.20 GB  (0.95%)
#----------------------------------------------------------------------------------------------------------------
# Total number of files in Files/bash/1-min/2025/07: 113,359 files, this is the size: ( 45.58 GB) | 112,704,305 record  1.92 GB
# Total number of files in Files/bash/1-min/2025/08: 200,398 files, this is the size: (199.89 GB) | 477,203,963 record  9.99 GB
# Total number of files in Files/bash/1-min/2025/09: 345,642 files, this is the size: (299.62 GB) | 714,897,310 record 18.99 GB
# Total number of files in Files/bash/1-min/2025/10: 355,759 files, this is the size: (322.68 GB) | 769,376,415 record 29.80 GB
# Total number of files in Files/bash/1-min/2025/11: 145,686 files, this is the size: (134.25 GB) | 320,512,258 record 34.19 GB

# Total number of files in Files/bash/Historical   :       1 files, this is the size: (107.96 GB) | 684,383,697 record  7.41 GB


# Total number of files in Files/iot-raw-data-json/bash/1-min/2026/02: 84,346 files, this is the size: 61,706,684,485 bytes (57.47 GB)
# Total number of files in Files/iot-raw-data-json/bash/1-min/2026/01: 288,337 files, this is the size: 172,097,257,514 bytes (160.28 GB)
...
table_name = "IIoT_Bronze_lakehouse.bash.iot_bash"
...
try:
    ...
    # print(f"Metadata Record Count: {record_count_fast:,}")
...
# ------------------------------------------------------------
lakehouse_folder = "Files/iot-raw-data-json/bash/1-min/2026/01"
...
# ------------------------------------------------------------
target_table = "IIoT_Bronze_lakehouse.bash.iot_bash"
(
    df.write
        .format("delta")
...
CSV_FOLDER_PATH = "Files/bash/Historical"
...
target_table = "IIoT_Bronze_lakehouse.bash.iot_bash_csv"
...
source_table = "IIoT_Bronze_Lakehouse.bash.iot_bash"
...
columns_to_drop = ["Id", "InstrumentID", "Quality_Text", "SignalId", "Timestamp", "source_file_path", "Value", "IsAlarm", "Quality", "Host","ValueDateAndTime"]
...
target_table = "IIoT_Silver_Lakehouse.bash.iot_bashXXX"
#overwrite   append
df_final.write \
     .format("delta") \
     .mode("overwrite") \
...
# 3️⃣ Read the source table
# source_table = "IIoT_Bronze_Lakehouse.bash.iot_bash_csv"
source_table = "IIoT_Bronze_Lakehouse.bash.bash_mongodb"
df = spark.read.table(source_table)

# 4️⃣ Take only top 100 records for testing
df_transformed = df#.limit(1000)
print(f"✅ Sample returned")

# Split into date and time columns
df_transformed = df_transformed.withColumn("signalDTTM", from_unixtime(col("timestamp")).cast("timestamp"))
...
# 8️⃣ Handle mixed-type 'Value' column
df_transformed = df_transformed.withColumn("value_Number",
...
df_transformed = df_transformed.withColumn("value_Text", lit(None).cast("string"))
df_transformed = df_transformed.withColumn(
    "value_Text",
    when(
        (col("value_Number").isNull()) &
        (col("value_DateTime").isNull()) &
        (col("value").isNotNull()),
...
#🔟 Write the transformed data to the Lakehouse table with schema merge
target_table = "IIoT_Silver_Lakehouse.bash.iot_bash"
#overwrite   append
df_final.write \
     .format("delta") \
     .mode("append") \
...
spark.sql("""
    DELETE FROM IIoT_Silver_lakehouse.bash.iot_bash
    WHERE signalDate = '2026-02-19'
""")
...
spark.sql("""
    INSERT INTO IIoT_Silver_lakehouse.bash.iot_bash
(asset, block, plant, section, signalDate, signalDTTM, signal, value_Number, value_DateTime, value_Text, value_Type)
SELECT 
    asset,
    block,
    plant,
    ...
    Value_Text,
    Value_Type
FROM IIoT_Silver_lakehouse.bash.iot_bashxxx
