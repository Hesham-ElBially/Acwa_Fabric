...
lakehouse_path = "Files/MongoDB/uaq"
...
# 357.79 GB | 233,494 file ===>>> 15.94 GB parquet (4.46%) | 504,159,329 record ===>>> 1,261,799,940 record 4.30 GB  (1.20%)
#-------------------------------------------------------------------------------------------------------------------------------
# Total number of files in Files/uaq/1-min/2025/07:  47,534 files, this is the size: ( 21.45 GB) |  53,275,403 record 0.78 GB
# Total number of files in Files/uaq/1-min/2025/08: 129,198 files, this is the size: (128.42 GB) | 313,381,602 record 5.38 GB
# Total number of files in Files/uaq/1-min/2025/09:  56,759 files, this is the size: ( 56.35 GB) | 137,502,324 record 7.63 GB

# Total number of files in Files/uaq/Historical/  :       1 files, this is the size: ( 52.62 GB) | 339,865,394 record 3.56 GB

# Total number of files in Files/MongoDB/uaq      :       2 files, this is the size: ( 98.95 GB) | 757,640,611 record 4.75 GB
...
table_name = "IIoT_Silver_Lakehouse.uaq.iot_uaq"
...
# ------------------------------------------------------------
lakehouse_folder = "Files/uaq/1-min/2025/09"
...
# ------------------------------------------------------------
target_table = "IIoT_Bronze_lakehouse.uaq.iot_uaq"
(
    df.write
        .format("delta")
...
CSV_FOLDER_PATH = "Files/uaq/Historical"
...
target_table = "IIoT_Bronze_lakehouse.uaq.iot_uaq_csv"
...
source_table = "IIoT_Bronze_Lakehouse.uaq.iot_uaq"
...
columns_to_drop = ["Id", "InstrumentID", "Quality_Text", "SignalId", "Timestamp", "source_file_path", "Value", "IsAlarm", "Quality", "Host", "_corrupt_record","ValueDateAndTime","ShouldBeAddedToCache"]
...
target_table = "IIoT_Silver_Lakehouse.uaq.iot_uaq"
#overwrite   append
df_final.write \
     .format("delta") \
     .mode("overwrite") \
...
# 3️⃣ Read the source table
source_table = "IIoT_Bronze_Lakehouse.uaq.iot_uaq_csv"
df = spark.read.table(source_table)
...
##############################################################################################
# This is to transform the table another structure MongoDB DATA
##############################################################################################
...
# 3️⃣ Read the source table
source_table = "IIoT_Bronze_Lakehouse.uaq.hist_uaq_mongodb"
df = spark.read.table(source_table)
...
# 9️⃣ Drop unwanted columns
columns_to_drop = ["Id", "instrumentID", "quality", "SignalId", "timestamp", "source_file_path", "value", "IsAlarm", "Quality", "Host", "_corrupt_record","timeField","ShouldBeAddedToCache"]
df_final = df_transformed.drop(*[c for c in columns_to_drop if c in df_transformed.columns])
