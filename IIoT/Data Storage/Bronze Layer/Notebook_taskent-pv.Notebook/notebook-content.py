...
lakehouse_path = "Files/taskent-pv/1-min/2025/10/"
...
# 46.56 GB | 56,138 file ===>>> 0.95 GB parquet 2.04% | 104,723,928 record ===>>> 0.15 GB  (0.32%)
# Friday, July 11, 2025	==>> Thursday, October 30, 2025
#----------------------------------------------------------------------------------------------------------------
# Total number of files in Files/taskent-pv/1-min/2025/07:  9,170 files, this is the size: (1.91  GB) |  4,526,174 record 0.04 GB
# Total number of files in Files/taskent-pv/1-min/2025/09: 11,701 files, this is the size: (11.65 GB) | 26,127,293 record 0.28 GB
# Total number of files in Files/taskent-pv/1-min/2025/10: 35,267 files, this is the size: (33.00 GB) | 74,070,461 record 0.95 GB
#
# Historical files: There was no historical files
#-----------------------------------------------------------------------------------------------------------------
# 
# MongoDB Extract
#-----------------------------------------------------------------------------------------------------------------
# Total number of files in Files/taskent-pv/1-min/2025/10: 1 files, this is the size: (10.2 GB) | 34,497,569 record 0.06 GB
...
table_name = "IIoT_Bronze_lakehouse.taskentpv.hist_taskentpv_mongodb"
...
# ------------------------------------------------------------
lakehouse_folder = "Files/taskent-pv/1-min/2025/10"
...
# ------------------------------------------------------------
target_table = "IIoT_Bronze_lakehouse.taskentpv.iot_taskentpv"
(
    df.write
        .format("delta")
...
CSV_FOLDER_PATH = "Files/taskentpv/Historical"
...
target_table = "IIoT_Bronze_lakehouse.taskentpv.iot_taskentpv_csv"
...
source_table = "IIoT_Bronze_Lakehouse.taskentpv.iot_taskentpv"
...
df_sample = (df#.filter(col("_corrupt_record").isNull())
                ...
                .withColumn("signal", col("SignalName"))
                #.limit(10)
            )
...
columns_to_drop = ["Id", "InstrumentID", "Quality_Text", "SignalId", "Timestamp", "source_file_path", "Value", "IsAlarm", "Quality", "Host", "_corrupt_record","ValueDateAndTime","SignalName"]
...
target_table = "IIoT_Silver_Lakehouse.taskentpv.iot_taskentpv"
#overwrite   append
df_final.write \
     .format("delta") \
     .mode("overwrite") \
...
# 3️⃣ Read the source table
source_table = "IIoT_Bronze_Lakehouse.taskentpv.hist_taskentpv_mongodb"
df = spark.read.table(source_table)
...
# Split into date and time columns
df_transformed = df_sample.withColumn(
    "signalDTTM",
