...
# CONFIG
# ------------------------------------------
source_folder = "Files/iot-raw-data-parquet/sirdarya/1-min"
...
WITH latest AS (
    SELECT *
    FROM `IIoT_Silver_lakehouse`.`sirdarya`.`iot_sirdarya`
    WHERE signalDate = (
    ...
    AND signal IN (
        SELECT signal
        FROM `IIoT_Silver_lakehouse`.`sirdarya`.`iot_sirdarya_chemistry`
    )
)
SELECT 
...
FROM `IIoT_Silver_lakehouse`.`sirdarya`.`iot_sirdarya_chemistry` etapro
...
# ------------------------------------------------------------
final_dir = "/lakehouse/default/Files/iot-api-data/FABRIC/sirdarya-chemistry"
...
WITH latest AS (
    ...
    AND signal IN (
        SELECT signal
        FROM `IIoT_Silver_lakehouse`.`sirdarya`.`iot_sirdarya_etapro`
    )
)
SELECT 
    CONCAT(c.plant, '.', c.section, '.', c.block, '.', c.asset, '.', c.signal) AS instrumentID,
    ...
    unix_timestamp(c.signalDTTM) AS timestamp,
    c.signalDTTM AS timeField
FROM `IIoT_Silver_lakehouse`.`sirdarya`.`iot_sirdarya_etapro` etapro
LEFT JOIN latest c ON etapro.signal = c.signal
"""
df = spark.sql(query)
...
# 4️⃣ Ensure folder exists BEFORE writing
# ------------------------------------------------------------
final_dir = "/lakehouse/default/Files/iot-api-data/sirdarya-etapro"
os.makedirs(final_dir, exist_ok=True)
...
WITH latest AS (
    ...
    AND signal IN (
        SELECT signal
        FROM `IIoT_Silver_lakehouse`.`sirdarya`.`iot_sirdarya_mpc`
    )
)
SELECT 
    CONCAT(c.plant, '.', c.section, '.', c.block, '.', c.asset, '.', c.signal) AS instrumentID,
    ...
    unix_timestamp(c.signalDTTM) AS timestamp,
    c.signalDTTM AS timeField
FROM `IIoT_Silver_lakehouse`.`sirdarya`.`iot_sirdarya_mpc` etapro
LEFT JOIN latest c ON etapro.signal = c.signal
"""
df = spark.sql(query)
...
# 4️⃣ Ensure folder exists BEFORE writing
# ------------------------------------------------------------
final_dir = "/lakehouse/default/Files/iot-api-data/FABRIC/sirdarya-mpc"
os.makedirs(final_dir, exist_ok=True)
