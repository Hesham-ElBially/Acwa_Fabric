...
# CONFIG
# ------------------------------------------
source_folder = "Files/iot-raw-data-parquet/rabigh-3/1-min"
...
# iot-api-data/FABRIC/rabigh3-etapro
#=========================================================================================
...
WITH latest AS (
    SELECT *
    FROM `IIoT_Silver_lakehouse`.`rabigh3`.`iot_rabigh3`
    WHERE signalDate = (
    ...
    AND signal IN (
        SELECT signal
        FROM `IIoT_Silver_lakehouse`.`rabigh3`.`iot_rabigh3_etapro`
    )
)
SELECT 
...
FROM `IIoT_Silver_lakehouse`.`rabigh3`.`iot_rabigh3_etapro` etapro
LEFT JOIN latest c ON etapro.signal = c.signal
"""
df = spark.sql(query)

# Optional: Show how many records Spark returned
# print("Spark count =", df.count())
...
# ------------------------------------------------------------
final_dir = "/lakehouse/default/Files/iot-api-data/FABRIC/rabigh3-etapro"
...
#=========================================================================================
# THIS IS to export to iot-api-data for other applications which is depending on mongoDB 
# iot-api-data/FABRIC/rabigh3-general
#=========================================================================================
...
WITH latest AS (
    ...
    AND signal IN (
        SELECT signal
        FROM `IIoT_Silver_lakehouse`.`rabigh3`.`iot_rabigh3_general`
    )
)
SELECT 
    CONCAT(c.plant, '.', c.section, '.', c.block, '.', c.asset, '.', c.signal) AS instrumentID,
    ...
    unix_timestamp(c.signalDTTM) AS timestamp,
    c.signalDTTM AS timeField
FROM `IIoT_Silver_lakehouse`.`rabigh3`.`iot_rabigh3_general` etapro
LEFT JOIN latest c ON etapro.signal = c.signal
"""
df = spark.sql(query)
...
# 4️⃣ Ensure folder exists BEFORE writing
# ------------------------------------------------------------
final_dir = "/lakehouse/default/Files/iot-api-data/FABRIC/rabigh3-general"
os.makedirs(final_dir, exist_ok=True)
...
WITH latest AS (
    ...
    AND signal IN (
        SELECT signal
        FROM `IIoT_Silver_lakehouse`.`rabigh3`.`iot_rabigh3_mpc`
    )
)
SELECT 
    CONCAT(c.plant, '.', c.section, '.', c.block, '.', c.asset, '.', c.signal) AS instrumentID,
    ...
    unix_timestamp(c.signalDTTM) AS timestamp,
    c.signalDTTM AS timeField
FROM `IIoT_Silver_lakehouse`.`rabigh3`.`iot_rabigh3_mpc` etapro
LEFT JOIN latest c ON etapro.signal = c.signal
"""
df = spark.sql(query)
...
# 4️⃣ Ensure folder exists BEFORE writing
# ------------------------------------------------------------
final_dir = "/lakehouse/default/Files/iot-api-data/FABRIC/rabigh3-mpc"
os.makedirs(final_dir, exist_ok=True)
